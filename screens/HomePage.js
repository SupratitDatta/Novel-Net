...
import booksData from "../constants/BookData.js";
...
const HomePage = ({ navigation }) => {
    ...
    const renderHeader = () => (
        <View style={{ flex: 1, flexDirection: 'row', paddingHorizontal: SIZES.padding }}>
            <View style={{ flex: 1 }}>
                <View style={{ marginRight: SIZES.padding, alignItems: "center", justifyContent: "center" }}>
                    <Text style={{ ...FONTS.h3, color: COLORS.white, marginTop: 20, fontSize: 20 }}>Hello There 👋</Text>
                </View>
            </View>
            <TouchableOpacity
                style={{
    ...
    const renderButton = (label, icon, onPress) => (
        <TouchableOpacity
            style={{ flex: 1 }}
            onPress={onPress}
        >
            <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }}>
                ...
                />
                <Text style={{ marginLeft: 5, ...FONTS.body3, color: COLORS.white}}>{label}</Text>
            </View>
        </TouchableOpacity>
    );
    ...
    const renderMyBookItem = ({ item, index }) => (
        <TouchableOpacity
            style={{
                flex: 1,
        ...
        >
            ...
            <View style={{ marginTop: SIZES.radius, flexDirection: 'row', alignItems: 'center' }}>
                ...
                <Image
                    ...
                    style={{ marginLeft: SIZES.radius, width: 20, height: 20, tintColor: COLORS.lightGray }}
                />
    ...
    const renderCategoryItem = ({ item }) => (
        ...
        >
            <Text style={{ ...FONTS.h2, color: selectedCategory === item.id ? COLORS.white : COLORS.lightGray }}>
                {item.categoryName}
            </Text>
        </TouchableOpacity>
    );
    ...
    const renderCategoryItemData = ({ item }) => (
        <TouchableOpacity
        ...
        >
            ...
            />
            <Text style={{ marginTop: SIZES.radius, ...FONTS.h3, color: COLORS.white }}>{item.bookName}</Text>
        </TouchableOpacity>
    );
